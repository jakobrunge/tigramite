...
def test_bootstrap_confidence(par_corr):
    # Set the random seed
    np.random.seed(5)
    # Use the given test from the pytest fixture
    par_corr = par_corr
    ...
    dim, T = 2, 1000
    array = np.random.multivariate_normal(mean=np.zeros(dim),
                                          cov=cov,
                                          size=T).T
    ...
    conf_ana = par_corr.get_analytic_confidence(df=T-dim, value=val,
