...
try:
    ...
    from sklearn.linear_model import MultiTaskLasso, MultiTaskLassoCV, Lasso, LassoCV, Ridge, RidgeCV         
...
class ParCorr(CondIndTest):
    ...
    **kwargs : 
    ...
    # @profile
    def _get_general_residuals(self, array, 
                standardize = False,
                return_means=False,):
        """Returns residuals of regularized linear multiple regression."""
        ...
        if dim_z > 0:
            ...
            if 'lasso' in self.regularizer:
                if 'lassocv' in self.regularizer:
                    model = MultiTaskLassoCV(cv=5, 
                        alphas=None, #numpy.array([0.0001, 0.0005, 0.001, 0.005, 0.01, 0.1, 1.]).astype('float'), 
                        fit_intercept=True, 
                        normalize=True) #, tol=0.0001) 

                elif 'lassofixalpha' in self.regularizer:
                    alpha = float(self.regularizer.split('_')[1])
                    # print self.regularizer.split('_'), alpha
                    model = MultiTaskLasso(alpha=alpha, 
                        fit_intercept=True, 
                        normalize=True, 
                        max_iter=1000) #, tol=0.0001) 
                else:
                    raise ValueError("regularizer %s unknown " % self.regularizer)

                model.fit(z, xy)
                x_pred, y_pred = model.predict(z).squeeze().T
                x_resid = xy[:,0] - x_pred
                y_resid = xy[:,1] - y_pred
                self.reduced_df = T - (model.coef_[0] != 0.).sum()

                if 'recompute' in self.regularizer:
                    # print model.coef_[0] != 0.
                    nonzero = list(2 + numpy.where(model.coef_[0] != 0.)[0])
                    # print nonzero
                    # print model.alpha_
                    # print len(model.coef_[0] != 0.), dim_z
                    nonzero_array = array[[0,1] + nonzero]
                    # print nonzero_array.shape
                    x_resid = self._get_single_residuals(nonzero_array, 0)
                    y_resid = self._get_single_residuals(nonzero_array, 1)


            elif 'ridge' in self.regularizer:
                if 'ridgecv' in self.regularizer:
                    model = RidgeCV(cv=5, 
                        alphas=(0.1, 1.0, 10.0),
                        fit_intercept=True, 
                        normalize=True) #, tol=0.0001) 
                ...
                x_resid = xy[:,0] - x_pred
                y_resid = xy[:,1] - y_pred
                realeigs = linalg.svdvals(z)
                self.reduced_df = int(T - (realeigs**2/(realeigs**2 + model.alpha_)).sum())

        else:
            x_resid, y_resid = xy.T
            mean = None
            self.reduced_df = T
    ...
    def get_dependence_measure(self, array, xyz):
        ...
        if self.regularizer is None:
            ...
            x, y = self._get_general_residuals(array)
...
if __name__ == '__main__':
    ...
    cond_ind_test = ParCorr(
        significance='analytic',
        sig_samples=100,
        regularizer= 'ridgecv',   #'lassofixalpha_0.01_recompute',
    ...
    Z = [(var, -lag) for var in range(N) for lag in range(1, tau_max+1)
                    ]
